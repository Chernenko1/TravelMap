...
export async function getPlaces(lat: number, lng: number, radius: number, categories: string[]) {
  ...
  try {
    const { data }: { data: GeoapifyResponse } = await axios.get(
      `https://api.geoapify.com/v2/places?categories=${sCategories}&filter=circle:${lng},${lat},${radius}&bias=proximity:${lng},${lat}&limit=${LIMIT}&apiKey=${import.meta.env.VITE_GEO_API_KEY}`,
    )
